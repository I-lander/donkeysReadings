...
<p>This application provides AI-generated tarot readings. It works without any user account.</p>
...
<ul>
<li>No account, no email, no name is collected.</li>
...
<p>Cette application fournit des lectures de tarot generees par IA. Elle fonctionne sans compte utilisateur.</p>
...
<ul>
<li>Aucun compte, aucun email, aucun nom n'est collecte.</li>
...
app.post('/api/generateReading', async (req, res) => {
  ...
  // Same question already asked by this device: return the stored reading
  // (same answer, same cards), without an OpenAI call or quota consumption.
  const previous = findReadingByQuestion(deviceId, question, language);
  if (previous) {
    res.status(200).json({
      result: previous.result,
      cards: JSON.parse(previous.cards) as DrawnCard[],
      quota: getQuota(deviceId),
      cached: true,
    });
    return;
  }
  ...
  try {
    const history = getReadingHistory(deviceId);
    ...
    saveReading(deviceId, question, language, JSON.stringify(cards), result);
    res.status(200).json({ result, cards, quota: getQuota(deviceId) });
  } catch (error) {
